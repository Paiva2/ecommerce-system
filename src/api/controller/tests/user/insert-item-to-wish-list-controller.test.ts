...
describe("Insert item to wish list controller", () => {
  beforeAll(async () => {
  ...
  it("should be possible to insert an item to user wish list.", async () => {
    ...
    const addToWishList = await request(app) // adding item to wish list
      .post(`/wish-list`)
    ...
    expect(addToWishList.statusCode).toEqual(204)
  ...
  it("should not be possible to insert an item on an wish-list without an auth token.", async () => {
    const userWishList = await request(app).post(`/wish-list`).send()
